...
class quadratic_lyapunov_nn(nn.Module):
    ...
    def forward(self,x):
        x1 = x[:, 0]  # First column, all rows (batch)
        x2 = x[:, 1]  # Second column, all rows (batch)
        res = -11.1403 * (x1 * x2) + 16.3896 * (x1 ** 2) + 11.4027 * (x2 ** 2)
...
class quad_norm(nn.Module):
    ...
    def forward(self, x):
        ...
        # quad_term = 2 * torch.sqrt(ATPDh_norm_sq) + torch.sqrt(self.P_norm_sq) * Dh_norm_sq
...
class h_nn(nn.Module):
    ...
    def forward(self,x):
        x1 = x[:, 0]  # First column, all rows (batch)
        x2 = x[:, 1]  # Second column, all rows (batch)
        res = 1 + 1/4 - ((x1 - 1)**2 + (x2 - 1)**2) / 0.5**2  
        return res.unsqueeze(-1)
    
class dynamics(nn.Module):
    def __init__(self, dt=0.1, mu=1.0):
    ...
    def forward(self, x, u=None):
        ...
        dx1 = x1 - self.dt * x2
        dx2 = x2 + self.dt * (x1 - self.mu * (1 - x1**2) * x2)
